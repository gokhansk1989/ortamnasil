    ...
    html: `
      <div style="font-family: 'Segoe UI', Tahoma, sans-serif; max-width: 440px; margin: 0 auto; padding: 32px 24px;">
        ...
        <div style="background: #F5F5F4; border-radius: 16px; padding: 28px; text-align: center;">
          ...
          <p style="color: #A8A29E; font-size: 13px; margin: 0 0 20px;">Giriş yaptıktan sonra yeni şifre belirlemen istenecek.</p>
          <a href="https://www.ortamnasil.com/giris" style="display: inline-block; background: linear-gradient(135deg, #F97316, #EC4899); color: #fff; font-weight: 700; font-size: 15px; padding: 12px 32px; border-radius: 12px; text-decoration: none;">
            Giriş yap
          </a>
        </div>
...
export function notifyAdmin(subject: string, lines: { label: string; value: string }[]): void {
  const rows = lines
    .map((l) => `<tr><td style="padding:6px 12px;color:#78716C;font-size:14px;white-space:nowrap">${l.label}</td><td style="padding:6px 12px;font-size:14px;color:#1C1917;font-weight:500">${l.value}</td></tr>`)
    .join("");
  ...
  sgMail
    .send({
      ...
      text,
      html: `
        <div style="font-family:'Segoe UI',Tahoma,sans-serif;max-width:500px;margin:0 auto;padding:24px">
          <h2 style="font-size:18px;color:#F97316;margin:0 0 16px">${subject}</h2>
          <table style="border-collapse:collapse;width:100%;background:#F5F5F4;border-radius:12px;overflow:hidden">${rows}</table>
          <p style="color:#A8A29E;font-size:12px;margin-top:16px;text-align:center">OrtamNasıl? Admin Bildirimi</p>
        </div>`,
    })
    .catch(() => {});
}
