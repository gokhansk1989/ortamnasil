...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const user = await prisma.user.findUnique({ where: { id: userId }, select: { nick: true } });
    notifyAdmin("Yeni itiraf yazıldı (onay bekliyor)", [
      { label: "Kullanıcı", value: user?.nick || "?" },
      { label: "Yurt", value: dorm.name },
      { label: "Başlık", value: title },
      { label: "Metin", value: text.slice(0, 100) + (text.length > 100 ? "…" : "") },
    ...
    return NextResponse.json({
      ...
      message: "İtiraf kaydedildi — yönetici onayından sonra yayınlanacak",
    }, { status: 201 });
  } catch {
